...
void main() {
  ...
  vec2 p = uScale * ((gl_FragCoord.xy - 0.5 * iResolution.xy) / unit) - 0.5;
...
export function MoltenMetal({
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      ...
      program.uniforms.uWorld.value = world > 1 ? world * scale : 0;
  ...
  useEffect(() => {
    ...
    u.uEnableMouse.value = mouseInteraction ? 1 : 0;
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    const dpr = ctx.renderer.dpr || 1;
    ctx.program.uniforms.uWorld.value = worldSize > 1 ? worldSize * dpr : 0;
  }, [worldSize]);
