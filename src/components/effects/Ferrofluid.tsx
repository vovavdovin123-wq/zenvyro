...
export type FerrofluidFlow = "up" | "down" | "left" | "right";
...
function paintColors(program: Program, input?: string[]) {
  const { arr, count, avg } = prepColors(input);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  float unit = uWorld > 1.0 ? uWorld : iResolution.y;
  vec2 p = fragCoord / unit * ref;
...
export function Ferrofluid({
  ...
  useEffect(() => {
    ...
    const followPointer = mouseInteraction && !coarse;
    const renderer = new Renderer({
      webgl: 1,
      dpr: dpr ?? glDpr(container.getBoundingClientRect().height),
      alpha: true,
      antialias: false,
    });
    ...
    const uniforms = {
      ...
      uMouseEnabled: { value: followPointer ? 1 : 0 },
      uMouseStrength: { value: mouseStrength },
      uMouseRadius: { value: mouseRadius },
      uWorld: { value: worldRef.current > 1 ? worldRef.current : 0 },
    };
    ...
    const resize = () => {
      ...
      uniforms.uWorld.value = world > 1 ? world * scale : 0;
    };
    ...
    let running = true;
    let lastDraw = 0;
    const frameGap = container.getBoundingClientRect().height > 800 ? 32 : 16;
    ...
    return () => {
      ...
      const callIfFn = (obj: object | null, key: string) => {
    ...
    // Context is created once; color uniforms update in a separate effect.
  ...
  useEffect(() => {
    ...
    const scale = renderer.dpr || 1;
    program.uniforms.uWorld.value = worldSize > 1 ? worldSize * scale : 0;
  }, [worldSize]);
