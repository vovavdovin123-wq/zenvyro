...
import { isCoarsePointer, isToolbarJitter } from "@/lib/glBudget";
import "./DotField.css";
...
export const DotField = memo(function DotField({
  ...
  useEffect(() => {
    ...
    function buildDots(w: number, h: number) {
      ...
      const spacing = p.dotSpacing * (coarse ? 1.7 : 1);
    ...
    function tryStart() {
      if (paused || !visible || rafRef.current) return;
    ...
    function tick() {
      ...
      const busy = !coarse && (eng > 0.002 || m.x > -9000);
      ...
      for (let i = 0; i < len; i += 1) {
        ...
        if (!coarse && distSq < crSq && eng > 0.01) {
      ...
      ctx.fill();
      if (coarse || (idleFrames > 18 && !busy && p.waveAmplitude <= 0)) {
        rafRef.current = 0;
        return;
      }
      rafRef.current = requestAnimationFrame(tick);
